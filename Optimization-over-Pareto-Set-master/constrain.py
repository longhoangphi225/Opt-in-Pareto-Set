...
class PNG_solver:
    ...
    def task(self, x, grad=False):
        ...
        if grad:
            # x.grad.data.zero_()
            #f1 = torch.log(x[0][0]**2 + torch.Tensor([1/4])*(x[0][1]-torch.Tensor([9/2]))**2+1)
            f1 = x[0][0]**2 + torch.Tensor([1/4])*(x[0][1]-torch.Tensor([9/2]))**2
            f1.sum().backward()
            grad1 = x.grad.data.detach().clone()
            x.grad.data.zero_()
            #f2 = torch.log(x[0][1]**2 + torch.Tensor([1/4])*(x[0][0]-torch.Tensor([9/2]))**2+2)
    ...
    def wd(self, x, context):
        F = self.task(x)
        energy = (1/context[0]) * (F[0,0] - context[0]) ** 2 + (1/context[1]) * (F[0,1] - context[1]) ** 2
        # energy = context[0] * (F[0, 0] - context[0]) ** 2 + context[1] * (F[0, 1] - context[1]) ** 2
        x.grad.data.zero_()
        energy.backward()
        energy_grad = x.grad.clone().detach()
    ...
    def quadratic_F(self, x, context):
        F = self.task(x)
        energy = context[0]*(F[0,0]-2)**2 + context[1]*(F[0,1]-2)**2
        x.grad.data.zero_()
        energy.backward()
        energy_grad = x.grad.clone().detach()
        return energy, energy_grad
    ...
    def complex_cos_F(self, x, context):
        ...
        #energy = - torch.cos(0.5 * 3.14159 * (F[0] - context[0])) + ((1. + torch.cos(3.14159 * (F[1] - context[1]))) ** 2)
        x.grad.data.zero_()
        energy.backward()
        energy_grad = x.grad.clone().detach()
        return energy, energy_grad
    ...
    def utility_F(self, x, context):
        ...
        energy = ((F[0,0]+4)**context[0])*((F[0,1]+4)**context[1])
        x.grad.data.zero_()
        energy.backward()
        energy_grad = x.grad.clone().detach()
        return energy, energy_grad
    ...
    def grad_search_constrain(self, energy_grads, task_grads, alpha=0.5, threshold=0.01):
        ...
        #threshold = 1e-1        # obtain mgd gradient
    ...
    #     for _ in range(100):
    #         hinge = constrain - torch.matmul(task_grads,(v_init.T))
    #         #norm2 = ((energy_grads*v_init).sum())/(torch.sqrt(torch.sum(torch.square(v_init))))
    #         norm2 = torch.sum(torch.square(energy_grads-v_init))
    #         loss = ((1/2) * norm2)+ (C)*torch.sum(torch.max(torch.zeros_like(hinge),hinge))
    #         loss.backward()
    #         #print(v_init.grad.data)
    #         v_init.data -= lr * v_init.grad.data
    #         v_init.grad.data.zero_()
    #     grad_constrain = v_init
    #     #print(grad_constrain.shape)
    #     return grad_constrain, betas * betas_mask
    ...
    def optimize(self, x, criterion='min norm', context=None, alpha=0.5, threshold=0.1, start=-1):
        ...
        if criterion == 'utility':
            ...
            e, de = self.quadratic_F(x, context)
        elif criterion == 'linear':
            e, de = self.linear_F(x, context)
        d, betas = self.grad_search_constrain(de, df, alpha=alpha, threshold=threshold)
        return betas
